#!/usr/bin/env bash
# Wrapper for preferences_save_persistence.yaml. pm-clear +
# _disable_dev_fab + dev-client rebind. Smoke seeds the test renter via
# runScript inside Maestro.
#
# Usage: cd mobile/.maestro && ./flows/smoke/run_preferences_save_persistence.sh
set -euo pipefail
...
cd "$SCRIPT_DIR/.."
exec bash run.sh flows/smoke/preferences_save_persistence.yaml
