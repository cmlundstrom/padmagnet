...
# Same pattern as run_l1_signin_signup_split.sh and run_renter_onboarding.sh:
# wipes app data + re-binds the dev client to Metro before invoking maestro,
# because clearState inside the YAML kills the dev client's Metro URL.
#
...
cd "$SCRIPT_DIR/.."
exec bash run.sh flows/smoke/auth_welcome_back_reactivation.yaml
