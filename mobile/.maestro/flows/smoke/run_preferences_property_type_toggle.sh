#!/usr/bin/env bash
# Wrapper for preferences_property_type_toggle.yaml. pm-clear +
# dev-client rebind. Smoke seeds the test renter via runScript inside
# Maestro. Same pattern as run_notifications_channel_select.sh.
#
# Usage: cd mobile/.maestro && ./flows/smoke/run_preferences_property_type_toggle.sh
set -euo pipefail
...
cd "$SCRIPT_DIR/.."
exec bash run.sh flows/smoke/preferences_property_type_toggle.yaml
