#!/usr/bin/env bash
# Wrapper for report_listing_submission.yaml. pm-clear + dev-client
# rebind. Smoke seeds the test renter via runScript inside Maestro.
# Same pattern as run_notifications_channel_select.sh.
#
# Usage: cd mobile/.maestro && ./flows/smoke/run_report_listing_submission.sh
set -euo pipefail
...
cd "$SCRIPT_DIR/.."
exec bash run.sh flows/smoke/report_listing_submission.yaml
