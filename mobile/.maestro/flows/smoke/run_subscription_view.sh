#!/usr/bin/env bash
# Wrapper for subscription_view.yaml. pm-clear + dev-client rebind.
# Smoke seeds named owner via runScript inside Maestro.
#
# Usage: cd mobile/.maestro && ./flows/smoke/run_subscription_view.sh
set -euo pipefail
...
cd "$SCRIPT_DIR/.."
exec bash run.sh flows/smoke/subscription_view.yaml
