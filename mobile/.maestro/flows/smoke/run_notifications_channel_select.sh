#!/usr/bin/env bash
# Wrapper for notifications_channel_select.yaml. pm-clear + dev-client
# rebind. The smoke itself seeds the test renter via runScript inside
# Maestro. Same pattern as run_settings_navigation.sh.
#
# Usage: cd mobile/.maestro && ./flows/smoke/run_notifications_channel_select.sh
set -euo pipefail
...
cd "$SCRIPT_DIR/.."
exec bash run.sh flows/smoke/notifications_channel_select.yaml
