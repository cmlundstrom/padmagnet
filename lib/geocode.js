...
/**
 * Geocode a free-form query (city name, zip code, etc.)
 * Used by tenant search zone creation.
 */
export async function geocodeQuery(query) {
  const key = process.env.GOOGLE_SERVER_GEOCODING_KEY;
  if (!key) {
    console.warn('GOOGLE_SERVER_GEOCODING_KEY not set — skipping geocoding');
  ...
  try {
    ...
    if (data.status === 'OK' && data.results[0]) {
      ...
      return { latitude: lat, longitude: lng, formatted_address: data.results[0].formatted_address };
    }
