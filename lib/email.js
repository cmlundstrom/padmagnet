 ...
 */
export async function sendNotificationEmail({ to, recipient_name: _rn, sender_name: _sn, listing_address: _la, message_preview: _mp, inbox_url, conversationId }) {
  // Escape all user-controlled values to prevent XSS in email HTML
  const recipient_name = escapeHtml(_rn);
  const sender_name = escapeHtml(_sn);
  const listing_address = escapeHtml(_la);
  const message_preview = escapeHtml(_mp);
 ...
 */
export async function sendExternalAgentEmail({ to, recipient_name: _rn, sender_name: _sn, sender_email: _se, sender_phone: _sp, message_time: _mt, listing_address: _la, message_preview: _mp, conversationId }) {
  // Escape all user-controlled values to prevent XSS in email HTML
  const recipient_name = escapeHtml(_rn);
  const sender_name = escapeHtml(_sn);
  const sender_email = escapeHtml(_se);
  const sender_phone = escapeHtml(_sp);
  const message_time = escapeHtml(_mt);
  const listing_address = escapeHtml(_la);
  const message_preview = escapeHtml(_mp);
