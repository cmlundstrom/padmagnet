 ...
 * @param {Array} [zones] - tenant_search_zones rows (optional, for multi-zone scoring)
 * @returns {{ score: number, factors: Array, explanation: string }}
 */
export function calculatePadScore(prefs, listing, zones) {
  ...
  } else if (prefs.center_lat && prefs.center_lng && prefs.radius_miles) {
    locSources.push({ lat: prefs.center_lat, lng: prefs.center_lng, radius: prefs.radius_miles, label: null });
  }
  ...
  for (const src of locSources) {
    ...
    if (dist <= src.radius) {
      ...
      factor = { key: 'location_inside_radius', label, impact: -penalty, match: true };
    } else {
      ...
      factor = { key: 'location_outside_radius', label, impact: -penalty, match: false };
    }
  ...
  if (prefs.association_preferred === false && listing.hoa_fee > 0) {
    totalPenalty += WEIGHTS.association_mismatch;
    factors.push({ key: 'association_mismatch', label: 'Has Association', impact: -WEIGHTS.association_mismatch, match: false });
  } else if (prefs.association_preferred === true && (!listing.hoa_fee || listing.hoa_fee === 0)) {
    totalPenalty += WEIGHTS.association_mismatch;
    factors.push({ key: 'association_mismatch', label: 'No Association', impact: -WEIGHTS.association_mismatch, match: false });
  } else if (prefs.association_preferred !== null && prefs.association_preferred !== undefined) {
    factors.push({ key: 'association_mismatch', label: 'Association Match', impact: 0, match: true });
  }
  ...
  if (prefs.min_lease_months && listing.lease_term) {
    const months = parseInt(listing.lease_term, 10);
    if (!isNaN(months) && months < prefs.min_lease_months) {
