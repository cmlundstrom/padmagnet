...
export function calculatePadScore(prefs, listing, zones) {
  ...
  if (prefs.association_preferred === false && listing.hoa_fee > 0) {
    ...
    factors.push({ key: 'association_mismatch', label: 'Association OK', impact: 0, match: true });
  }
  ...
  if (listing.previous_list_price && listing.price_changed_at && listing.list_price < listing.previous_list_price) {
    const daysSinceDrop = Math.floor((Date.now() - new Date(listing.price_changed_at).getTime()) / (1000 * 60 * 60 * 24));
    if (daysSinceDrop <= 7) {
