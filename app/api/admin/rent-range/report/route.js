...
export async function POST(request) {
  ...
  try {
    ...
    const streetViewUrl = getStreetViewUrl(report, pd);
...
function getStreetViewUrl(report, pd) {
  if (!GOOGLE_KEY) return null;
  const lat = pd.lat || report.property_details?.lat;
  const lng = pd.lng || report.property_details?.lng;
  const address = report.property_address;

  // Prefer coordinates, fallback to address
  const location = (lat && lng) ? `${lat},${lng}` : encodeURIComponent(`${address}, ${report.city}, ${report.state} ${report.zip}`);

  return `https://maps.googleapis.com/maps/api/streetview?size=600x300&location=${location}&fov=90&pitch=10&key=${GOOGLE_KEY}`;
}
